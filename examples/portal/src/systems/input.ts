 ...
 *
 * Basis at yaw 0: forward = -Z, right = +X. Rotating by `yaw` keeps W/D moving
 * toward what the player is looking at. Velocity is *set* (not accumulated), so
 * there is no horizontal momentum yet — that arrives with the portal/teleport
 * milestone, which needs "speedy thing comes out" to survive a frame.
 */
export const inputSystem: SchedulableSystem<GameState> = {
  ...
  run(ctx) {
    ...
    if (grounded.onGround) {
      ...
      // Limited air control: nudge toward the input but keep momentum, so a
      // portal fling actually carries you across.
