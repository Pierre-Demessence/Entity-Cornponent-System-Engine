...
| 7 | `modules/spawner` | **Build** (new) | 4 (flappy, jetpack, space-invaders, top-down-shooter) | M | Cadence + difficulty-ramp emitter on a **repeating `Timer`** (now shipped); 4 consumers, solid canon. Directly useful for the next shooter. Absorbs jetpack's interval timers from the original #6 scope. |
