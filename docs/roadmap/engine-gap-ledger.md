...
| Off-screen / boundary recycling & culling — despawn-or-wrap entities that leave a region; cull off-screen entities from update/render. | flappy, jetpack, asteroids, frogger (wrap), top-down-shooter, tilemap (cull) | **wrap/clamp PRESENT** (`applyBoundary`@[`motion/motion.ts`](../../src/modules/motion/motion.ts), `boundary:{mode}`); **despawn** trivial (`lifetime` + `queueDestroy`); **render cull ABSENT** (viewport reserved for `camera`, README:178). | **Split.** Genuine gap = **cull off-screen from render** → backlog `camera` V2 view-rect + render-canvas2d viewport cull. (audit B1) |
...
| Boundary clamp — constrain an entity to a region (clamp position to bounds + zero velocity on breach). | flappy, jetpack, breakout, rpg (+ paddle/actor clamps in breakout, frogger, local-pong, space-invaders) | **PARTIAL**: `boundary:{mode:'clamp'}`@`motion/motion.ts` clamps the position **origin to `[0,width)`/`[0,height)` only** — `applyBoundary` has **no inset / per-entity bounds / size offset**. Full-playfield clampers (asteroids, top-down-shooter) already adopt it; size/margin-aware clampers (breakout paddle `[minX,maxX]`, frogger `[0,W-FROG]`, local-pong, space-invaders) **cannot** express their bounds with it. | **Split (corrected 2026-07-15)**: full-playfield clamp = adoption (the 2 that fit); size-aware clamp = a **Build**, now in backlog as `modules/motion` boundary **inset/per-entity** extension. (audit B4) |
| Fisher–Yates shuffle + random-pick-from-tag. Pure, domain-neutral utility. | card-battler, solitaire, snake (`randomEmptyCell`) | **ABSENT**: no `modules/rng`. | → backlog NEW `modules/rng` (shuffle + pick + optional seed). (audit B6) |
...
| Velocity normalize / set-speed-in-direction — `hypot` rescale to a target magnitude. | breakout, top-down-shooter | **ABSENT**: no `normalize`/`setSpeed`/`hypot` util in `motion/`. | → backlog extend `modules/motion` (vector normalize / set-speed util). (audit B13) |
