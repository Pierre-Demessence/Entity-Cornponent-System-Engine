...
| AABB-vs-AABB overlap test. Engine ships `circleVsCircle` only; consumers hand-write the rect-vs-rect overlap predicate. | jetpack | Open | Sibling of the circle-vs-AABB gap; a shared `aabbVsAabb` / `circleVsRect` collision-primitive module would cover both. |
| Timed-lifetime (TTL) entities. No engine "live for N ms then despawn" component, so consumers hand-roll an `ageMs`/`lifeMs` component plus an age-and-`queueDestroy` reaper system (bullets, particles). | jetpack | Open | Recurs anywhere there are particles/projectiles; compare a `LifetimeDef` + reaper primitive. |
