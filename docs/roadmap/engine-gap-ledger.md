...
| Spawn timer / cadence with difficulty ramp — "emit an entity every T ms, where T lerps with difficulty." | flappy, jetpack, space-invaders, top-down-shooter | **PARTIAL**: no `modules/spawner` yet, but its timing core (`modules/timer` repeating `Timer`) shipped 2026-07-16. | → backlog NEW `modules/spawner` = repeating `Timer` + ramp + emit-callback. Absorbs jetpack's `bulletTimerMs`/`spawnTimerMs` interval-emitters (re-homed from B3). (audit B2) |
