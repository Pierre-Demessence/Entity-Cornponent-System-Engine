...
>
> Status: **awaiting human triage.** Nothing here is actioned yet.

> ⚠️ **Stale ledger:** `engine-gap-ledger.md`'s AABB row says *"Engine ships
> `circleVsCircle` only"* — that's wrong. `aabbVsAabb`, `aabbVsCircle` and
> `aabbVsAabbSwept` all already exist in
> [`src/modules/collision/narrowphase.ts`](../../src/modules/collision/narrowphase.ts).
> Fix that row; the real gap (A1 below) is **adoption**, not a missing primitive.
...
| --- | --- | --- | --- | --- |
| A1 | Rect/circle overlap test | `aabbVsAabb`, `aabbVsCircle`, `aabbVsAabbSwept` — [`collision/narrowphase.ts`](../../src/modules/collision/narrowphase.ts) | local-pong, flappy (`circleVsRect`), frogger, jetpack, space-invaders | **STRONG** — ✅ **DONE** (all 5 migrated) |
...
| A3 | High-score persistence | `save` module — `src/modules/save/` | breakout, jetpack, frogger, flappy, snake (raw `localStorage`) | ~~STRONG~~ → **NOT A GAP** (see note) |
...
| A5 | Camera follow / world↔view xform | `makeFollowCameraSystem`, `worldToView`/`viewToWorld` — [`camera/camera.ts`](../../src/modules/camera/camera.ts) | tilemap (CSS transform), top-down-shooter (unused), rpg (manual clamp) | ~~MEDIUM~~ → **NOT A GAP** (see note) |
...
| A7 | Canvas2D renderer | `render-canvas2d` | local-pong, snake (raw `ctx2d` calls) | ~~MEDIUM~~ → **NOT A CLEAN GAP** (see note) |
...
> but mixes two render paths for marginal benefit; left as-is.
