...
- **`quatForward`** _(fn)_ `(q: Quat): Vec3` — The direction the local `-Z` axis points after applying `q` — "forward" in a right-handed coordinate system. In a left-handed system wher...
...
- **`quatNormalize`** _(fn)_ `(q: Quat): Quat` — Unit-length copy of `q`. A zero-length input returns the identity rather than `NaN`: a zero quaternion is not a rotation.
...
- **`Vec2`** _(interface)_ — A plain 2D vector value object.
- **`vec2MoveToward`** _(fn)_ `(current: Vec2, target: Vec2, delta: number): Vec2` — `current` stepped `delta` units along the straight line to `target`; sibling of `vec3MoveToward`. A forward mover cannot overshoot: once ...
...
- **`vec3MoveToward`** _(fn)_ `(current: Vec3, target: Vec3, delta: number): Vec3` — `current` stepped `delta` units along the straight line to `target` — the `Vec3` sibling of `vec2MoveToward`. A forward mover cannot over...
...
- **`Opacity`** _(interface)_ — Per-entity opacity multiplier in [0, 1]. Renderer applies via `globalAlpha`.
...
- **`RenderOrder`** _(interface)_ — Per-entity z-order. Renderer draws entities in ascending `value`; ties preserve component-store insertion order.
