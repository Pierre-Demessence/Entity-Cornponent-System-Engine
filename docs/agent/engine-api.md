...
- **`vec3MoveToward`** _(fn)_ — `current` advanced at most `delta` units along the straight line to `target` — the `Vec3` sibling of `modules/motion`'s `moveToward`, mat...
- **`vec3Negate`** _(fn)_ — `-v`.
...
- **`moveToward`** _(fn)_ — `current` advanced at most `delta` units along the straight line to `target` — Unity `Vector2.MoveTowards`, Godot `Vector2.move_toward`, ...
