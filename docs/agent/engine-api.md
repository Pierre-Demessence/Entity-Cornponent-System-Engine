...
- **`InputRawEvent`** _(type)_ `{ kind: 'down'; code: string; } | { kind: 'up'; code: string; }` — Low-level "where do raw input events come from" contract. Core ships the interface; concrete providers (keyboard, gamepad, touch, synthet...
