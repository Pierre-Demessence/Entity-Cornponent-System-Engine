...
- **`action`** _(fn)_ — Leaf: run `fn` and use its status. Return `success` for an instant effect, `running` for a behaviour that spans ticks (travel, wait), or ...
- **`BtNode`** _(type)_ — A tree node: a pure function of the (per-agent) context returning a status. Reactive by design — the whole tree is re-ticked from the roo...
- **`BtStatus`** _(type)_ — A behaviour-tree tick result. `running` means the node needs more ticks to finish (e.g. an agent still travelling to a target); composite...
- **`condition`** _(fn)_ — Leaf: `success` when `pred` is true, else `failure`.
- **`inverter`** _(fn)_ — Swap `success` ⇄ `failure`; `running` passes through.
- **`selector`** _(fn)_ — Run children in order until one does not fail (logical OR / fallback). Returns `success` on the first child that succeeds, `running` on t...
- **`sequence`** _(fn)_ — Run children in order (logical AND). Returns `failure` on the first child that fails, `running` on the first that is running, and `succes...
...
- **`Aabb3Axis`** _(type)_ — —
- **`aabb3ContainsPoint`** _(fn)_ — True when `p` lies within the box or exactly on its boundary.
- **`aabb3VsAabb3`** _(fn)_ — True when the boxes' three axis projections all overlap. Edge contact does NOT count.
- **`aabb3VsAabb3Swept`** _(fn)_ — Swept AABB: `a` moves by `motionA` toward static `b`. Returns the fraction of the motion at which they first touch (`tEntry ∈ [0, 1]`) an...
- **`aabb3VsObb3`** _(fn)_ — Axis-aligned box vs oriented box — the same SAT, with the first box seen as unrotated.
- **`aabb3VsPlane3`** _(fn)_ — Box–plane overlap: the box straddles the plane when its centre is no further from the plane than the box's own extent along the plane nor...
- **`aabb3VsSphere3`** _(fn)_ — Box–sphere overlap: the closest point on the box to the sphere centre lies within the radius.
- **`Obb3`** _(interface)_ — A world-space oriented box: centre, **half** extents, and its rotation.
- **`obb3VsObb3`** _(fn)_ — Oriented-box vs oriented box by the separating-axis theorem: the two sets of three face normals plus the nine cross products of their axe...
- **`obb3VsSphere3`** _(fn)_ — Oriented-box vs sphere: transform the centre into the box's frame, then it is the axis-aligned test.
...
- **`plane3DistanceToPoint`** _(fn)_ — Signed distance from the plane to `p` — a true distance only for a unit `normal`.
- **`RayHit3`** _(interface)_ — —
- **`rayVsAabb3`** _(fn)_ — Ray vs centre-based AABB, by the slab method. Returns the entry distance and the axis of the entry face, or `null` when the ray misses, t...
- **`rayVsObb3`** _(fn)_ — Ray vs oriented box. Same contract as `rayVsAabb3` — including the `t` convention that makes `t <= 1` a segment test — but the reported `...
- **`rayVsPlane3`** _(fn)_ — Ray vs plane: the `t` at which the ray meets the plane, or `null` when the ray runs parallel to it or the plane lies behind the origin. `...
...
- **`sphere3ContainsPoint`** _(fn)_ — True when `p` lies within the sphere or exactly on its surface.
- **`sphere3VsPlane3`** _(fn)_ — Sphere–plane overlap: the centre is no further from the plane than the radius.
- **`sphere3VsSphere3`** _(fn)_ — Two spheres overlap (or touch) when the centre distance is at most the sum of radii.
...
- **`plan`** _(fn)_ — Find the least-cost sequence of `actions` that transforms `initial` into a state satisfying `goal`, via A* over symbolic world-states. Re...
- **`WorldState`** _(type)_ — A flat set of boolean world facts (F.E.A.R.-style symbolic state). A missing key reads as `false`. Preconditions and effects are partial ...
...
- **`createEventInput`** _(fn)_ — Wires one or more `InputProvider`s to an action map and returns an event-dispatched `EventInput`. Edges are **action-level** and identica...
...
- **`InputEvent`** _(type)_ — A committed action transition handed to `EventInput.subscribe` handlers — the event-mode counterpart of `InputState.justPressed` / `justR...
...
- **`KeyboardEmitSource`** _(type)_ — Which `KeyboardEvent` field supplies the emitted `code`. - `'code'` (default) — physical key position, labelled after US-QWERTY. It is la...
- **`KeyboardProvider`** _(class)_ — DOM keyboard adapter. Emits raw `down`/`up` events keyed by either `KeyboardEvent.code` (default) or `KeyboardEvent.key` — see `KeyboardP...
...
- **`MouseLookProvider`** _(class)_ — Reports relative pointer motion while the pointer is locked to `target`, and tracks the lock itself. It binds no input gesture of its own...
...
- **`quatConjugate`** _(fn)_ — The conjugate `(w, -x, -y, -z)` — the inverse rotation for a **unit** quaternion, and the cheapest way to undo one. A non-unit `q` would ...
- **`quatForward`** _(fn)_ — The direction the local `-Z` axis points after applying `q` — the "forward" of the right-handed convention (three.js, Bevy, Godot). Unity...
- **`quatFromAxisAngle`** _(fn)_ — Rotation of `angle` radians about `axis`, right-handed: curl the fingers of your right hand around the axis and the thumb points along th...
- **`quatMul`** _(fn)_ — Hamilton product `a·b`: the rotation that applies `b` **first**, then `a`. Composing with `quatMul` is how angular velocity is integrated...
- **`quatNormalize`** _(fn)_ — Unit-length copy of `q`. A zero-length input returns the identity rather than `NaN`: a zero quaternion is not a rotation, and three.js re...
- **`quatRotate`** _(fn)_ — Rotate `v` by the **unit** quaternion `q`. A non-unit `q` scales the result instead of rotating it, so normalise — or compose only unit q...
- **`quatSlerp`** _(fn)_ — Spherical linear interpolation from `a` to `b`: the blend that turns at a **constant angular speed**, so an eased camera sweep does not a...
- **`quatUp`** _(fn)_ — The direction the local `+Y` axis points after applying `q`.
...
- **`Vec2`** _(interface)_ — A plain 2D vector value object.
- **`vec2MoveToward`** _(fn)_ — `current` stepped `delta` units along the straight line to `target` — Unity `Vector2.MoveTowards`, Godot `Vector2.move_toward`, Unreal `F...
- **`vec2Normalize`** _(fn)_ — Unit vector pointing the same direction as `v`. A zero-length input has no direction, so it returns `{ x: 0, y: 0 }` rather than `NaN` — ...
- **`vec2ScaleToLength`** _(fn)_ — `v` rescaled so its length equals `length`, direction preserved — the canonical "normalize then multiply" for driving a body at a fixed s...
- **`Vec3`** _(interface)_ — A plain 3D vector value object.
- **`vec3Add`** _(fn)_ — `a + b`, component-wise.
- **`vec3AddScaled`** _(fn)_ — `a + b * s` — the accumulate an integrator wants. One call instead of a `vec3Scale` allocation plus an add.
- **`vec3ClampLength`** _(fn)_ — `v` clamped to at most `max` in length, direction preserved — the spherical bounds test. Returns `v` itself when it is already short enou...
- **`vec3Cross`** _(fn)_ — Right-handed cross product: the vector a right-hand rule curls `a` into `b` about, with length `|a||b|sin θ`.
- **`vec3Distance`** _(fn)_ — Distance between two points.
- **`vec3Dot`** _(fn)_ — Dot product. Zero exactly when the two are perpendicular.
- **`vec3Length`** _(fn)_ — Euclidean length.
- **`vec3LengthSq`** _(fn)_ — Squared length — use it when you only need to compare distances, to skip the square root.
- **`vec3Lerp`** _(fn)_ — Component-wise linear interpolation; `t` is **unclamped** (compose with `modules/math`'s `clamp01` for a bounded blend).
- **`vec3MoveToward`** _(fn)_ — `current` stepped `delta` units along the straight line to `target` — the `Vec3` sibling of `vec2MoveToward`, matching Unity `Vector3.Mov...
- **`vec3Negate`** _(fn)_ — `-v`.
- **`vec3Normalize`** _(fn)_ — Unit vector pointing the same way as `v`. A zero-length input has no direction, so it returns `{ 0, 0, 0 }` rather than `NaN` — a caller ...
- **`vec3RandomUnit`** _(fn)_ — A uniformly distributed point **on** the unit sphere — every result has length 1, unlike a naive per-axis random in `[-1, 1]` which biase...
- **`vec3Reflect`** _(fn)_ — Reflect `v` off a surface whose normal is the **unit** vector `normal`, via `v - 2(v·n)n`. A non-unit normal does not just rescale the re...
- **`vec3Scale`** _(fn)_ — `v` scaled by `s`.
- **`vec3ScaleToLength`** _(fn)_ — `v` rescaled so its length equals `length`, direction preserved — the canonical "normalize then multiply" for driving a body at a fixed s...
- **`vec3Sub`** _(fn)_ — `a - b`, component-wise.
...
- **`fbm3D`** _(fn)_ — Fractal Brownian motion, 3D — the 3D sibling of `fbm1D` / `fbm2D`.
...
- **`Noise3D`** _(type)_ — 3D coherent-noise sampler over a seed. Returns a value in `[-1, 1]` for finite input, up to `perlin3D`'s two caveats (see its doc).
...
- **`perlin3D`** _(fn)_ — Perlin gradient noise, 3D — the canonical construction, and exactly `0` at every integer lattice point. The gradients are the 12 canonica...
...
- **`simplex3D`** _(fn)_ — Ken Perlin's simplex noise, 3D — gradient noise on a tetrahedral lattice, the 3D sibling of `simplex2D` and the usual pick for volumetric...
...
- **`valueNoise3D`** _(fn)_ — Value noise, 3D: faded trilinear interpolation of hashed lattice values. `valueNoise3D(x, y, 0, s)` is `valueNoise2D(x, y, s)` bit for bi...
...
- **`CircleAnchor`** _(type)_ — What `PositionDef` anchors to on a `circle`: its centre or its bounding box's top-left.
...
- **`RectAnchor`** _(type)_ — What `PositionDef` anchors to on a `rect`: its top-left corner or its centre.
- **`Renderable`** _(type)_ — Discriminated union of drawable shapes the default Canvas2D renderer understands. Anchor convention: - `rect`: `anchor` chooses `'top-lef...
