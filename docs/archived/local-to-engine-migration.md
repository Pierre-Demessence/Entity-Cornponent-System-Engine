...
[engine-gap-ledger](engine-gap-ledger.md) triage (exhaustive source-cited
pass) and the [module backlog](ecs-module-backlog.md).

> **Re-cited against the stamped ledger, then dual-side verified.** Three pre-correction entries were wrong: (1) the old #3
...
4. **Targeted extensions (8, 10).** `collision` reflection (8) and
   `camera` V2 camera-consume (10) — both shipped. (#3 shipped as `modules/math` — reshaped
   from the boundary-inset extension; see the queue row.)
5. **Overlay pass (9).** The new screen-space layer — shipped.
