...
- `lifetime/` — `LifetimeDef { remainingMs }` component plus a `makeLifetimeSystem<TCtx>(options?)` factory that counts down each tick by `ctx.dtMs` and destroys expired entities (or invokes an optional `onExpire` hook). Import via `@pierre/ecs/modules/lifetime`. Canon pattern: Unreal `AActor::SetLifeSpan`, Unity `Destroy(obj, t)`, Gregory *Game Engine Architecture* §12.5.
