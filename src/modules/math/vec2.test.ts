...
import { vec2MoveToward, vec2Normalize, vec2ScaleToLength } from './vec2';
...
describe('vec2ScaleToLength', () => {
  it('rescales to the requested length, preserving direction', () => {
    const v = vec2ScaleToLength({ x: 3, y: 4 }, 10);
  ...
  it('handles an already-unit input', () => {
    const v = vec2ScaleToLength({ x: 1, y: 0 }, 7);
    expect(v).toEqual({ x: 7, y: 0 });
  });

  it('handles a negative-axis direction', () => {
    const v = vec2ScaleToLength({ x: 0, y: -1 }, 3);
    expect(v.x).toBe(0);
    expect(v.y).toBeCloseTo(-3);
  });

  it('returns the zero vector for a zero-length input', () => {
    expect(vec2ScaleToLength({ x: 0, y: 0 }, 100)).toEqual({ x: 0, y: 0 });
  });

  it('a diagonal input is not longer than a cardinal one', () => {
    const diag = vec2ScaleToLength({ x: 1, y: 1 }, 5);
    expect(Math.hypot(diag.x, diag.y)).toBeCloseTo(5);
  });
});

describe('vec2MoveToward', () => {
  it('steps the full delta along the line to the target', () => {
    const v = vec2MoveToward({ x: 0, y: 0 }, { x: 10, y: 0 }, 3);
    expect(v).toEqual({ x: 3, y: 0 });
  });

  it('steps diagonally at the requested distance', () => {
    const v = vec2MoveToward({ x: 0, y: 0 }, { x: 3, y: 4 }, 2.5);
  ...
  it('lands exactly on the target when delta equals the distance', () => {
    expect(vec2MoveToward({ x: 0, y: 0 }, { x: 3, y: 4 }, 5)).toEqual({ x: 3, y: 4 });
  });

  it('does not overshoot when delta exceeds the distance', () => {
    expect(vec2MoveToward({ x: 0, y: 0 }, { x: 3, y: 4 }, 100)).toEqual({ x: 3, y: 4 });
  });

  it('stays put on a zero distance', () => {
    expect(vec2MoveToward({ x: 2, y: 2 }, { x: 2, y: 2 }, 1)).toEqual({ x: 2, y: 2 });
  });

  it('settles on the target across repeated calls', () => {
    let p = { x: 0, y: 0 };
    for (let i = 0; i < 10; i++)
      p = vec2MoveToward(p, { x: 4, y: 0 }, 1);
    expect(p).toEqual({ x: 4, y: 0 });
  });

  it('leaves the position unchanged for delta = 0', () => {
    expect(vec2MoveToward({ x: 1, y: -1 }, { x: 5, y: 5 }, 0)).toEqual({ x: 1, y: -1 });
  });
  ...
  it('moves away from the target for a negative delta, as Unity and Godot do', () => {
    ...
    const v = vec2MoveToward({ x: 0, y: 0 }, { x: 0, y: 10 }, -3);
  ...
  it('agrees with the 3D twin on the planar case', () => {
    // Same inputs, same arithmetic — the two blocks must not drift apart.
    // (1,1) → (4,5) is a 3-4-5 triangle, so half the distance is (2.5, 3).
    const stepped = vec2MoveToward({ x: 1, y: 1 }, { x: 4, y: 5 }, 2.5);
